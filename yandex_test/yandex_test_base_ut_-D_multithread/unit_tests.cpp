...
/**
 * @brief Check validation of all blocks
 * 
...
static bool ValidateAllBlocks(const std::vector<struct BLOCK_CONTEXT>& allocatedBlocksContext) {
    ...
    for (const auto& elem : allocatedBlocksContext) {
        ...
        for (size_t i = 0; i < BLOCK_SIZE; ++i) {
            std::cout
                << "Byte#" << i << ": "
                << "correct value: " << elem.value
                << " block data: " << ((unsigned char*)elem.pBlock)[i]
                << std::endl;
...
static bool ReleaseAllBlocksWithValidation(
    ...
    while (allocatedBlocksContext.size() > 0) {
        size_t vSize = allocatedBlocksContext.size();
        // validate all allocated blocks
        if (!ValidateAllBlocks(allocatedBlocksContext)) {
            std::cerr << "Error: Blocks validation Fail" << std::endl;
            return false;
        }
        ...
        if (!Allocator_FreeBlock(allocatedBlocksContext[eraseElemNumber].pBlock)) {
            std::cerr
                << "Error: Allocator_FreeBlock(void *pBlock) return false during correct release"
                << std::endl;
            return false;
        }
        std::cout
...
bool CorrectUseTest(size_t passNum) {
    ...
    if ((allocatorIninResult == true && blocksQty == 0) || (allocatorIninResult == false && blocksQty > 0)) {
        std::cerr << "Allocator_Init() Error: return check FAILURE" << std::endl;
        return false;
    }

    // Allocate and fill blocks by randome data
    // and release allocated blocks in random order and validate all left blocks every release
...
bool IncorrectUseTest() {
    ...
    if (Allocator_GetBlock() != NULL) {
        std::cerr << "Error: Overflow allocation return not NULL" << std::endl;
        return false;
    }
    ...
    if (Allocator_FreeBlock((unsigned char*)allocatedBlocksContext.front().pBlock - 1) ||
        ...
        std::cerr << "Error: Success release PTR outside the pool" << std::endl;
        return false;
    }
    ReleaseAllBlocksWithValidation(allocatedBlocksContext);
    ...
    if (BLOCK_SIZE > 1) {
        ...
        AllocateAndFillAllBlocks(allocatedBlocksContext, blocksQty);
        if (Allocator_FreeBlock((unsigned char*)allocatedBlocksContext.front().pBlock + 1)) {
            std::cerr << "Error: Success release PTR which is not start of block" << std::endl;
            return false;
        }
        ReleaseAllBlocksWithValidation(allocatedBlocksContext);
    }
    ...
    if (Allocator_FreeBlock(pBlock)) {
        std::cerr << "Error: Success release of already released block" << std::endl;
        return false;
    }
    return result;
}